    ...
    public void run() {
        if (this.downloadLength < this.block) {
            ...
            try {
                ...
                Logger.d(TAG, "Thread " + this.threadId + " start download from position " + startPos);
                ...
                while ((len = inStream.read(buffer, 0, buffer.length)) != -1) {
                    ...
                    Logger.d(TAG, "Thread" + this.threadId + " downloadLength=" + this.downloadLength + ", len=" + len);
                    if (isInterrupt) {                this.downloadLength = -1;
                        Logger.d(TAG, "Thread " + this.threadId + " download interrupt.");
                        break;
                    }
                }
                if(!isInterrupt){
                    Logger.d(TAG, "Thread " + this.threadId + " download finish");
                    this.finish = true;
                }
            } catch (Exception e) {
                this.downloadLength = -1;
                Logger.d(TAG, "Thread " + this.threadId + ":" + e.getMessage());
            } finally {
                try {
